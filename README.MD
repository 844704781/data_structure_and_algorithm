...
1. 先用递归的方式去右子树中找出最小的节点
2. 再用递归的方式调用删除方法将其删除

注意：虽然这里调用了delete，delete也调用了findMinAndDel,但是delete中一定会递归到一个叶子结点，然后向上补充
...
2. 如果被查找的节点min_node是该节点父节点的左子节点，则父节点的左子节点设置为null
...
红黑树是一种节点带有颜色属性的二叉排序树，但他在二叉排序树之外，还有以下5大性质
